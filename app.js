...
class Queen {
    ...
    solveQueen = async (board, r, n) => {
        ...
        for (let i = 0; i < n; ++i) { //column iteration
            await q.delay();
            ...
            if (await q.isValid(board, r, i, n)) {
                ...
                row.getElementsByTagName("td")[i].innerHTML = queen; //visualisation of the queen on the board

                this.position[board][r] = i; //storing the position of the queen in the current board

                if (await q.solveQueen(board, r + 1, n))
                    await q.clearColor(board);

                await q.delay();
                board = Board; //updating the board value to the latest board
                ...
                row.getElementsByTagName("td")[i].innerHTML = "-"; //removing the queen from the board
...
playButton.onclick = async function visualise() {
    ...
    if (n > 8) {
        ...
        alert(`No solutions exist for ${n}x${n} board.`);
        return;
    }
